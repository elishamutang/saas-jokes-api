...
test('admin level or higher can search for a role based on name', function () {
    ...
    // Get role
    $searchKeyword = 'this';
    $response = $this->getJson("/api/v2/admin/roles?search=$searchKeyword");
...
test('admin level or higher can create a new role', function() {
    ...
    // Send POST request
    $response = $this->postJson("/api/v2/admin/roles", $data);
...
test('admin level or higher can update an existing role', function() {
    ...
    // Send POST request
    $response = $this->putJson("/api/v2/admin/roles/{$role->id}", $updatedRole);
