...
test('staff level and higher can paginate users', function() {
    ...
    // Get users
    $perPage = 2;
    $response = $this->getJson("/api/admin/users?per_page=$perPage");
...
test('staff level and higher can search for user based on name or email', function() {
    ...
    // Get user
    $searchKeyword = 'Doe';
    $response = $this->getJson("/api/admin/users?search=$searchKeyword");
...
test("clients can update own user name and email", function() {
    ...
    // Update user
    $response = $this->putJson("/api/admin/users/$userId", $updatedUser);
...
test("staff can update client user name and email", function() {
    ...
    // Update user
    $response = $this->putJson("/api/admin/users/{$selectedUser->id}", $updatedUser);
...
test('super-admin cannot be deleted', function() {
    ...
    // Delete user
    $response = $this->deleteJson("/api/admin/users/{$authUser->id}");
...
test('staff can only browse soft-deleted client users', function() {
    ...
    // Browse trash
    $response = $this->getJson("/api/admin/users/trash");
...
test('staff can recover client user', function() {
    ...
    $response = $this->postJson("/api/admin/users/trash/recover/{$newUser->id}");
...
test('admin can remove staff user or lower', function() {
    ...
    // Recover deleted user
    $response = $this->postJson("/api/admin/users/trash/remove/{$newUser->id}");
